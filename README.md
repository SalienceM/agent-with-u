...
- **单次委托确认** — 手动发送前开启输入工具栏的“Kit 代确认”开关，开启时显示“仅本次”，不再单独占一行。可授权 Agent 核对并批准一个运行/链中的发布步骤，6 小时内有效。服务端绑定当前用户、Session、运行范围、发布配置和计划指纹；聊天正文不产生授权，队列/语音/下一次发送不继承开关，切换会话或刷新也不会保留待发送授权。授权过期或计划/配置变化需重新授权。该能力不会自动唤醒已结束的聊天；长打包完成后可再发消息让 Agent 查询并核对，原运行授权仍在有效期内。
...
- **Agent Skills 市场** — 浏览/搜索公开 GitHub Skill 源，支持独立填写 `main`、`master` 或自定义分支（显式分支不自动回退）。安装前查看来源、许可证、文件清单、`SKILL.md` 预览与风险提示；右侧可切换“原文 / AI 中文解读”，按用途、用法、依赖与注意事项解释内容。解读可选择启用的 OpenAI 兼容 / Anthropic API Backend，使用无工具的独立文本请求；不会启动 Agent、安装或执行 Skill。相同内容的结果短期复用，失败可重试，解读不代替原文审核。
...
- **刷新恢复工作区** — 按当前用户保存完整 Tab 顺序、选中项与分屏焦点，刷新不会只剩正在显示的会话；后台 Tab 首次点开才加载。会话信息读取有超时和重试，断线重连或其他窗口组件取得新元数据后，保活的聊天面板会同步恢复，不再一直显示“正在恢复会话”。
...
市场支持大仓库：ZIP 流式下载到临时磁盘，仓库下载上限为 1 GiB；单个 Skill
展开后上限为 256 MiB / 20,000 个文件，单文件仍为 16 MiB。目录只缓存说明与
文件元数据，安装时复核指纹并完整导入选中 Skill 的配套目录，不导入无关展示项目。
加载和安装在执行端后台运行，界面显示下载进度，不阻塞聊天；关闭市场不会取消已提交任务。
大文件清单支持搜索与每页 100 条浏览。临时 ZIP 缓存最多 4 个 / 2 GiB，15 分钟过期，
后续请求清理过期缓存；正在读取的快照会保留到操作结束。正常退出时清理进程拥有的临时文件。
导入后的运行检查使用同一组资源限额，依赖安装仍需单独审核确认。
本地安装按钮同时接受：
