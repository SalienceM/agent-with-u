...
const SECTIONS: ManualSection[] = [
  {
    id: 'workspace-collaboration', title: '跨节点工作区协作', icon: '🌐', intro: '直接在普通对话中协调自己的节点、Session 和文件。',
    items: [{ icon: '🌐', title: '让聊天操作 AWU', summary: '可以询问可见节点，按名称查找 Session、搜索文件、分段读取文本，再按模板生成文件并落到目标节点。重名时返回候选，不会猜测；查询不会执行写操作。',
      entry: '普通对话；Session → 绑定能力 → Prompts → 跨节点工作区协作',
      tips: '默认启用并按需调用，不预载节点历史。控制窗口和目标节点均需保持连接并升级到支持版本。仅支持 Session 工作区内 UTF-8 文本，每批最多 100 个文件、1 MiB；不处理旧 SSH Session、二进制文档或凭据文件。',
      steps: [
        { title: '发现', text: '例如：帮我看看现在能访问哪些节点，再找 work_1 中的需求文件 A。' },
        { title: '组合', text: '例如：参考 home 节点「项目规范」Session 中的 template.md，把 A 每个需求拆成一个文件，在 home 新建一个 Session 存放全部结果。' },
        { title: '确认', text: '当前节点、当前 Session 的普通写入遵循输入工具栏「跳过确认」，无需额外的跨工作区确认。跨 Session／跨节点写入及新建 Session 则单独展示实际目标、Backend 和文件清单；确认一次只允许这一批新增，不覆盖、不删源文件、不自动执行新会话。「跳过后续」不扩展到其他 Session 或节点。' },
        { title: '回执', text: '成功后返回文件校验记录和可点击 Session 链接。断线不代表停止已提交写入；用原 requestId 查询或重试，不要新建重复任务。' },
      ] }],
  },
  {
    ...
    items: [
      ...
      { icon: '↑', title: '回看输入历史', summary: '输入框首行按 ↑ 回看最近 10 条输入；↓ 向后翻，Escape 返回原草稿。刷新、关闭应用再打开仍可恢复，只填入、不自动发送。', entry: '当前 Session 输入框 ↑ / ↓', tips: '历史保存在当前客户端，按用户、节点和 Session 隔离；只保留文字，不恢复附件或 Kit 代确认。旧会话会从已加载的用户消息补回缺失历史。删除 Session 会清理本机对应历史；清除浏览器／应用本地数据后不保证恢复未进入聊天记录的输入。' },
  ...
  },
  {
    ...
    items: [
      ...
      { icon: '✨', title: 'Prompts 提示词', summary: '把常用指令保存成可复用模板，在不同会话快速插入。', entry: '扩展 → Skills 与 Prompts → Prompts 页签' },
      { icon: '🧩', title: 'Skills 仓库父级管理', summary: '能力库的 Skills / Prompts 分页使用完整窗口宽度，支持搜索。仓库与独立 Skill 统一为紧凑列表：点击仓库名称展开子项，行尾「···」展开改名、默认档、维护手册和运行准备等操作。同仓库自动归为 Parent（只有一个子项也一样）；无仓库来源的本地 Skill 保持独立。搜索命中子项仍保留完整父组，不改变整组操作范围。', entry: '扩展 → Skills 与 Prompts；Session → 绑定能力', keywords: 'Parent 父级 仓库 分组 OpenSpec 全选 单独 启用 关联',
        tips: '绑定能力勾选父级，一次启用当前全部子项；展开可关闭某个子项，或用「仅此项」只保留本组的某一个，其他仓库不受影响。旧会话的部分绑定会显示实际数量，不自动全开。之后新安装的子项需再次选择，避免悄悄扩大执行权限。「整组设为默认」作用于当前子项和以后新建的 Session，不修改旧会话。父级改名不改子 Skill 名称、slash 命令或源文件，名称和手册保存在当前执行节点。父级引用请在俺寻思输入 @SKILL 并从菜单选择，显示名称旁携带稳定引用，无需记 ID；将汇总所有已安装子项资料和父级总手册。' },
      { icon: '📦', title: '按仓库批量安装 Skill', summary: '先在市场顶部「安装到」选择节点，再选择仓库并点击「安装此仓库全部」。新打开市场优先使用当前 Session 节点，可独立修改，不改变全局默认连接。搜索只过滤显示，不会缩小批次范围。批次确认后目标锁定，关闭弹框不取消任务；需取消未提交批次或完成结果查看后才能换节点。网页版的直连节点是服务器，不是浏览器所在电脑。', entry: '扩展 → 扩展市场 → 安装到 → 安装此仓库全部', keywords: '节点 本机 远端 仓库 全装 批量 安装 市场 布局 运行准备 弹窗',
        tips: '核对完整清单、来源、执行节点与文件目录后提交；每批最多 500 项。默认跳过当前版本，保留同名冲突和本地修改；需要替换时单独勾选覆盖。仓库内重名条目需逐个选择。执行端逐项导入并汇总成功、跳过、失败，失败可单独重试；回执未知时重查原批次，不重复提交。关闭批次窗口不停止任务，同一市场页可用「查看安装批次」继续查看；刷新页面或重启执行端不保证保留回执。安装只导入文件，不运行 Skill、不自动启用，不再逐项弹出运行准备。需要时点击「查看运行准备」；已就绪无需重复准备，实际依赖下载仍需核对计划后确认。' },
      { icon: '📖', title: 'Skill 使用手册与 @SKILL 答疑', summary: '能力库每行的「手册 ↗」打开独立阅读窗口；「··· → 维护手册」编辑人类使用说明。维护内容与原始 README / SKILL.md 分开保存，更新或重新安装 Skill 不会覆盖你的手册。', entry: '扩展 → Skills 与 Prompts → 手册 ↗；俺寻思输入 @SKILL', keywords: '使用文档 手册 维护 原始资料 窗口 弹窗 预览',
        tips: '在俺寻思输入 @SKILL 选择父级（默认）或展开子项后提问，Skill 是当前 Session／项目的附加参考资料，不会替换注意力焦点或另开问答线程。引用仅本条有效：发送成功后清除，下次不主动引用就不会自动追加或重读手册；发送失败保留草稿供重试。删除草稿中的引用或点击「清除 Skill 参考」立即移除本条资料，历史问答不受影响。引用不要求启用 Skill，也不会安装或执行。最多引用 3 个父级/子项，展开后最多 100 份资料，合计最多 36K 字符、单份最多 12K；各子项公平分配正文预算并标注截断，过大的组会提示缩小范围。父级总手册补充整体流程，不替代子项资料。可同时粘贴截图答疑（最多 8 张、合计 16 MiB、单张 2000 万像素），模型需支持图片，仍禁用工具。显式引用时答疑会结合会话摘要、当前界面、执行节点的有限项目资料（工作目录浅层清单及 README、项目清单、OpenSpec 配置片段）和实际绑定命令，不会扫描整仓库或执行 CLI。文档中的 CLI 命令不等于聊天 slash 已接入。未固定时切换 Session 会清空草稿和图片，固定时仍留在原项目，但不会把已发送的引用沿用到下一条。旧手册问答归回所属 Session。原始文档可切换渲染/源码，安全预览不会执行脚本或加载外部资源。多窗口保存冲突保留草稿并提示核对。浏览器需允许本站弹窗；桌面创建失败会显示原因并保留原面板。' },
      { icon: '/', title: 'Skill 命令与 OpenSpec 工作流', summary: '市场安装后，在 Session 的「绑定能力」启用。聊天输入 / 会列出本会话的 Skill；通用格式为 /skill 技能名 参数。选择命令只填入输入框，确认参数后发送，结果仍显示在当前聊天。', entry: '右键 Session → 绑定能力 → Skills；当前聊天输入 /', keywords: 'openspec opsx slash 斜杠 命令 缺少 CLI 项目 初始化',
        ...
        steps: [
          ...
          { title: '2. 项目初始化与维护入口', text: '当前 Session 的执行节点安装了 OpenSpec 工作流 Skill 后，主聊天输入 /opsx- 才会出现这一组项目入口；未安装不显示，卸载后刷新或重开菜单即移除。项目准备入口无需绑定，具体工作流仍需在 Session 启用。CLI 仍需自行安装。/opsx-init 初始化当前 Session 项目；/opsx-update 更新已有适配文件，不升级 CLI；/opsx-list 列出变更；/opsx-status <change-id> 查看工件状态；/opsx-show <id> 查看详情；/opsx-validate [id|--all|--specs|--changes] 校验文档（默认全部）；/opsx-help [子命令] 与 /opsx-version 查询实际帮助和版本。选择只填草稿，再发送才由 Agent 执行。' },
          { title: '初始化的边界', text: '/opsx-init 默认使用 --tools none，由 AWU 继续管理已绑定 Skill，不重复生成 Agent 适配文件。已初始化请勿重复 init；有残留目录或父项目时先检查，不自动覆盖。发送时检查当前节点的 Backend PATH / 项目 node_modules/.bin，缺少 CLI 报 OPENSPEC_CLI_MISSING，不自动下载；其他工作流缺少 openspec/config.yaml 时提示使用 /opsx-init。Agent 必须核验实际版本、help 支持和项目根，结果回到当前聊天。俺寻思能解释这些入口，但不执行或自动转发；应用 /init、/status 含义不变。' },
